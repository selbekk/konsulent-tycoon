...
for (let seed = 1; seed <= seeds; seed++) {
  ...
  for (let q = 0; q < 40; q++) {
    ...
    if (verbose && seed === 1 && q % 4 === 0) {
      ...
      console.log(
        `Q${q} firms ${firms.length} cap ${cap} active ${active} ratio ${(active / cap).toFixed(2)} util p10 ${u[Math.floor(u.length * 0.1)]?.toFixed(2)} med ${u[Math.floor(u.length / 2)]?.toFixed(2)}`,
      )
    }
    s = { ...endTurn({ ...s, status: 'playing' }), status: 'playing' }
  }
  for (const f of Object.values(s.firms))
    if (!f.isPlayer && f.bankrupt) {
      deaths[f.id] = (deaths[f.id] ?? 0) + 1
      totalDeaths++
    }
}
...
console.log(
  ...
  ratios
    .filter((_, i) => i % 4 === 0)
    .map((r) => med(r).toFixed(2))
    .join(' '),
)
console.log(
  `deaths/game ${(totalDeaths / seeds).toFixed(1)}`,
