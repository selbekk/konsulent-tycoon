...
export function crisisStyleFor(firm: Firm): CrisisStyle {
  ...
  const hush =
    CRISIS_AI_HUSH.base +
    CRISIS_AI_HUSH.perShadiness * p.shadiness +
    CRISIS_AI_HUSH.perCarelessness * (1 - p.qualityFocus)
...
function value(state: GameState, firm: Firm, c: Crisis, choice: CrisisChoice, style: CrisisStyle): number {
  ...
  const money =
    (e.cash ?? 0) + (e.cashPerHead ?? 0) * hc + (e.contractRevenue ?? 0) * perHead * 3 - benched * perHead * 0.4
  const soft = 0.5 + style.care
  const outcome =
    typeof choice.outcome === 'string' ? choice.outcome : choice.outcome?.[c.revealed ? c.severity : 'high']
  return (
    (money / revenue) * 100 +
    ...
    ((e.relationship ?? 0) +
      (e.satisfaction ?? 0) +
      (e.satisfactionAll ?? 0) * Math.min(3, activeContracts(state, firm.id).length)) *
      0.25 -
    (e.rateCut ?? 0) * 40 -
...
export function planCrisisAnswers(state: GameState, firmId: string, style?: CrisisStyle): Action[] {
  ...
  return openCrises(state, firmId).flatMap((c) => {
    ...
    const resolve: Action = {
      ...
      crisisId: c.id,
      choiceId: best.id,
      ...(best.talk ? { score: s.talk } : {}),
    }
    // A talk only counts once started, same as for the player.
    return best.talk
      ? [{ type: 'startCrisisTalk', firmId, crisisId: c.id, choiceId: best.id } as Action, resolve]
      : [resolve]
  })
}
