...
const activeCustomer: EventDef['params'] = ({ state, firm }, pick) => {
  const ids = state.contracts
    .filter(
      (c) => c.firmId === firm.id && !c.terminated && c.startQuarter <= state.quarter && state.quarter < c.endQuarter,
    )
    .map((c) => c.customerId)
...
export const EVENTS: EventDef[] = [
  ...
  },
  {
    ...
    weight: 1,
    cooldown: 4,
    condition: ({ state }) => state.quarter % 4 === 0,
    choices: [
  ...
  {
    id: 'leaked_salary_sheet',
    weight: 0.8,
    cooldown: Infinity,
    minQuarter: 4,
    condition: ({ headcount }) => headcount >= 10,
    choices: [
    ...
    condition: ({ state, firm }) =>
      state.contracts.some(
        (c) =>
          ...
          state.customers[c.customerId]?.sector === 'public',
      ),
    choices: [
  ...
  },
  {
    ...
    weight: 1,
    cooldown: 3,
    condition: ({ firm }) => firm.stars.length > 0,
    choices: [
  ...
  },
  {
    ...
    weight: 3,
    cooldown: 3,
    condition: ({ utilization }) => utilization > 0.95,
    choices: [
  ...
  {
    id: 'star_wants_remote',
    weight: 2,
    cooldown: 2,
    params: ({ firm }, pick) => {
      const stars = firm.stars.filter(
        (s) => !s.remoteGranted && (s.ambition === 'remote' || s.traits.includes('remote_hardliner')),
      )
      if (!stars.length) return null
      const s = pick(stars)
      return { starId: s.id, name: s.name }
  ...
  },
  {
    ...
    cooldown: 8,
    minQuarter: 8,
    condition: ({ firm }) => firm.cash > 8_000_000,
    choices: [
  ...
  {
    id: 'influencer_post_viral',
    weight: 0.7,
    cooldown: 6,
    choices: [{ id: 'nice', effect: { brand: 8, reputation: 2 } }],
  },
  {
    id: 'office_ping_pong',
    weight: 0.8,
  ...
  },
  {
    ...
    weight: 1,
    cooldown: 4,
    condition: ({ state }) => state.quarter % 4 === 2,
    choices: [
  ...
  },
  {
    ...
    weight: 5,
    cooldown: 4,
    condition: ({ state }) => state.quarter % 4 === 3,
    choices: [
  ...
  },
  {
    ...
    weight: 2,
    cooldown: 4,
    condition: ({ state }) => state.quarter % 4 === 1,
    choices: [
  ...
  },
  {
    ...
    weight: 0.6,
    cooldown: 8,
    condition: ({ firm }) => firm.fagmiljo > 50,
    choices: [{ id: 'nice', effect: { reputation: 4, brand: 4 } }],
  },
  { id: 'cloud_credit_windfall', weight: 0.5, cooldown: 8, choices: [{ id: 'nice', effect: { cash: 200_000 } }] },
  {
    ...
    weight: 2,
    cooldown: 8,
    condition: ({ morale, headcount }) => morale < 45 && headcount >= 15,
    choices: [
