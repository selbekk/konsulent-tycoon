...
Nivåsystemet (`engine/levels.ts`) og idéene som hang på det er bygget: feiring ved hvert nytt nivå, nøkkeltall fra nivå 2, kontor som vokser per nivå, mål per nivå (`content/missions.ts`) og Strategi-fanen med spesialisering, partnerskap, lobbying, avdelinger, oppkjøp og børsnotering. Tall og funn står i `docs/balance-log.md`.

Mulige neste steg:
- La AI-firmaene bruke spesialisering og avdelinger, så strategien ikke bare er spillerens fordel.
- Vurdere om oppkjøp og børsnotering sammen løfter en flink spiller for mye (se balanseloggen).
